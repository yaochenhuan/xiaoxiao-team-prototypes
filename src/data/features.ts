      ...
      sections: [
          ...
          blocks: [
            ...
            tbl(
              ...
              [
                ...
                ['<span class="badge-red" style="color:#dc2626;font-weight:600">Must Have</span>', '学情统计区', '我的进步、错题巩固归因、知识点掌握度建议 TOP 3'],
        ...
        // ========== 业务角色与权限 ==========
        {
          ...
          blocks: [
            ...
            h3('权限边界'),
            ul([
              '学生 A 不可访问学生 B 的错题数据（接口层按 student_id 过滤）。',
          ...
          blocks: [
            ...
            tbl(
              ...
              [
                ...
                ['学情统计区', '上方 3 张卡', '① 我的进步（折线 + 数值）② 错题巩固（环形 + 4 类归因）③ 知识点掌握度建议 TOP 3', 'M002'],
            ...
            h3('交互逻辑'),
            ol([
              '页面加载 → 并行请求 <code>GET /api/v1/wrong-questions/stats</code> + <code>GET /api/v1/knowledge/tree</code> + <code>GET /api/v1/wrong-questions?page=1</code>。',
              '用户切换考试类型 Tab → 重新触发上述 3 个接口，列表区显示 loading skeleton 0.5s 内完成切换。',
              '用户点击知识点二级节点 → 列表请求带上 <code>knowledge_id</code> 参数，表格内联筛选，无 toast。',
              '用户点击筛选项 → 单选确认后关闭下拉，<code>GET</code> 自动带上新参数。',
              '用户勾选错题行 → 底部批量栏"开始练习"在选中项 ≥ 1 时蓝紫高亮，否则置灰。',
              '用户点击行内星标 → 立即乐观更新 + 调 <code>POST /api/v1/wrong-questions/{id}/star</code>；toast 反馈"已收藏"或"已取消收藏"。',
            ]),
            h3('数据来源'),
            p('<code>GET /api/v1/wrong-questions/stats?exam_type=...</code> · <code>GET /api/v1/knowledge/tree?exam_type=...</code> · <code>GET /api/v1/wrong-questions?exam_type=...&knowledge_id=...&level=...&format=...&difficulty=...&page=...&page_size=20</code>'),
            h3('状态变化'),
        ...
        // ========== P002 基础训练 ==========
        {
          ...
          blocks: [
            ...
            ol([
              ...
              '点击"标记掌握" → 立即乐观更新状态 + 调 <code>POST /api/v1/wrong-questions/{id}/master</code>。',
            ]),
            h3('数据来源'),
            p('<code>GET /api/v1/training-questions?exam_type=...&knowledge_id=...</code> · <code>POST /api/v1/wrong-questions/{id}/master</code>'),
          ],
        },
...
└──────────────────────────────────────────┘`,
            ...
            h3('交互逻辑'),
            ol([
              '页面加载 → 调 <code>GET /api/v1/wrong-words/stats</code> + <code>GET /api/v1/wrong-words?page=1</code>。',
              ...
              '用户点击星标 → 立即乐观更新 + 调 <code>POST /api/v1/wrong-words/{id}/star</code>。',
              '用户点击单词卡片 → 展开单词详情（弹层 v0.2）。',
            ]),
            h3('数据来源'),
            p('<code>GET /api/v1/wrong-words/stats</code> · <code>GET /api/v1/wrong-words?page=...&sort=...&is_star=...&keyword=...</code> · <code>POST /api/v1/wrong-words/{id}/star</code>'),
          ],
        },
        ...
        // ========== 模块设计说明 ==========
        {
          ...
          blocks: [
            ...
            h3('M002 — 学情统计区'),
            p('原型绑定：P001 / P002 上方 3 张卡片。包含：① 我的进步（折线图 + 数值，正负号 + 整数 %）；② 错题巩固（环形 + 4 类归因百分比）；③ 知识点掌握度建议 TOP 3。'),
            h3('M003 — 知识点分类树'),
            p('原型绑定：P001 / P002 左侧 280px 区域。一级节点：三角箭头 ▶/▼ 表示展开/收起；二级节点：色块 + 文字 + 数量徽标；选中态：浅蓝底 + 蓝紫文字。'),
            h3('M004 — 错题列表 + 筛选'),
        ...
        {
          id: 'db-schema',
          title: '工程系统设计 · 数据库设计（DB Schema）',
          blocks: [
            h3('3.1.1 错题主表 wrong_questions'),
            code(
`CREATE TABLE wrong_questions (
  id            BIGINT       PRIMARY KEY AUTO_INCREMENT,
  student_id    BIGINT       NOT NULL                COMMENT '学生 ID',
  question_id   BIGINT       NOT NULL                COMMENT '原始题目 ID',
  exam_type     VARCHAR(32)  NOT NULL                COMMENT '新化班2026/托福/雅思/SAT/ACT/GRE/SAT机考/KET/PET/A-Level/AP',
  subject       VARCHAR(32)                          COMMENT '阅读/听力/口语/写作/数学/物理/化学/生物/历史/地理/政治',
  level         VARCHAR(16)                          COMMENT 'Lower/Medium/Upper/A1/A2/B1/B2/C1/C2/-',
  format        VARCHAR(64)                          COMMENT 'Complete the Words/Select Words/Single Choice/Multiple Choice/Fill Blank/Short Answer/Essay',
  difficulty    TINYINT                              COMMENT '1-5',
  knowledge_id  BIGINT                               COMMENT '知识点 ID',
  err_count     INT          DEFAULT 1               COMMENT '错课次数',
  err_reason    VARCHAR(64)                          COMMENT '审题不慎/粗心/不会做/时间不够/计算错误',
  is_star       TINYINT       DEFAULT 0               COMMENT '是否收藏',
  status        VARCHAR(16)   DEFAULT '待复习'        COMMENT '待复习/已掌握/特加强',
  last_err_at   DATETIME                             COMMENT '最近一次错误时间',
  created_at    DATETIME      DEFAULT CURRENT_TIMESTAMP,
  updated_at    DATETIME      DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX idx_student_exam (student_id, exam_type),
  INDEX idx_knowledge (knowledge_id),
  INDEX idx_status (status),
  INDEX idx_last_err (last_err_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='错题主表';`,
              'sql',
            ),
            h3('3.1.2 错词主表 wrong_words'),
            code(
`CREATE TABLE wrong_words (
  id            BIGINT       PRIMARY KEY AUTO_INCREMENT,
  student_id    BIGINT       NOT NULL                COMMENT '学生 ID',
  word          VARCHAR(50)  NOT NULL                COMMENT '单词',
  phonetic_us   VARCHAR(64)                          COMMENT '美式音标',
  phonetic_uk   VARCHAR(64)                          COMMENT '英式音标',
  cn_meaning    TEXT                                 COMMENT '中文释义',
  example_en    TEXT                                 COMMENT '英文例句',
  example_cn    TEXT                                 COMMENT '中文例句',
  err_count     INT          DEFAULT 1               COMMENT '错误数',
  is_star       TINYINT      DEFAULT 0               COMMENT '是否收藏',
  source        VARCHAR(16)  DEFAULT 'wrong'         COMMENT 'wrong/wordbook',
  created_at    DATETIME     DEFAULT CURRENT_TIMESTAMP,
  updated_at    DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uk_student_word (student_id, word),
  INDEX idx_is_star (is_star)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='错词主表';`,
              'sql',
            ),
            h3('3.1.3 错词-错题关联表 wrong_word_questions'),
            code(
`CREATE TABLE wrong_word_questions (
  id                 BIGINT   PRIMARY KEY AUTO_INCREMENT,
  wrong_word_id      BIGINT   NOT NULL,
  wrong_question_id  BIGINT   NOT NULL,
  created_at         DATETIME DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uk_wq (wrong_word_id, wrong_question_id),
  INDEX idx_question (wrong_question_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='错词与错题多对多关联';`,
              'sql',
            ),
            h3('3.1.4 学情统计缓存表 student_wrong_stats'),
            code(
`CREATE TABLE student_wrong_stats (
  id              BIGINT       PRIMARY KEY AUTO_INCREMENT,
  student_id      BIGINT       NOT NULL,
  exam_type       VARCHAR(32)  NOT NULL,
  week_done       INT          DEFAULT 0          COMMENT '本周错题数',
  total_done      INT          DEFAULT 0          COMMENT '累计错题数',
  consolidation   JSON                              COMMENT '归因分布 {审题不慎:38, 粗心:23, ...}',
  knowledge_tips  JSON                              COMMENT '掌握度建议 TOP 3',
  updated_at      DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uk_student_exam (student_id, exam_type)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='学情统计预聚合（每小时刷新）';`,
              'sql',
            ),
          ],
        },
        ...
        {
          id: 'api-design',
          title: '工程系统设计 · API 设计（OpenAPI 级）',
          blocks: [
            h3('3.2.1 接口总览'),
            tbl(
              ['接口名称', 'Method', 'Endpoint', 'SLA', '幂等性', '鉴权'],
              [
                ['学情统计', 'GET', '/api/v1/wrong-questions/stats', '<200ms', 'GET 天然幂等', '学生 token'],
                ['知识点树', 'GET', '/api/v1/knowledge/tree', '<200ms', 'GET 天然幂等', '学生 token'],
                ['错题列表', 'GET', '/api/v1/wrong-questions', '<500ms', 'GET 天然幂等', '学生 token'],
                ['收藏错题', 'POST', '/api/v1/wrong-questions/{id}/star', '<300ms', 'X-Idempotency-Key', '学生 token'],
                ['标记掌握', 'POST', '/api/v1/wrong-questions/{id}/master', '<300ms', 'X-Idempotency-Key', '学生 token'],
                ['开始练习', 'POST', '/api/v1/wrong-questions/practice', '<500ms', 'X-Idempotency-Key', '学生 token'],
                ['错词统计', 'GET', '/api/v1/wrong-words/stats', '<200ms', 'GET 天然幂等', '学生 token'],
                ['错词列表', 'GET', '/api/v1/wrong-words', '<500ms', 'GET 天然幂等', '学生 token'],
                ['收藏错词', 'POST', '/api/v1/wrong-words/{id}/star', '<300ms', 'X-Idempotency-Key', '学生 token'],
                ['家长汇总', 'GET', '/api/v1/parent/wrong-stats', '<500ms', 'GET 天然幂等', '家长 token'],
                ['教师汇总', 'GET', '/api/v1/teacher/wrong-stats', '<500ms', 'GET 天然幂等', '教师 token'],
              ],
            ),
            h3('3.2.2 错题列表 API 详细规格'),
            code(
`// GET /api/v1/wrong-questions
// Query
{
  exam_type:     string  (required)   // 考试类型
  knowledge_id?: number              // 知识点筛选
  level?:        string              // 级别
  format?:       string              // 题型
  difficulty?:   number              // 1-5
  sort?:         'err_count' | 'last_err_at' | 'word_az'   // 默认 err_count desc
  is_star?:      '0' | '1'           // 只看收藏
...
// Response 200
{
  ...
  data: {
    list: [
      {
        id: 1001,
        content: '...',                 // 题目内容（截断 200 字 + "展开"）
        subject: '阅读',
        level: 'B2',
        format: 'Single Choice',
        difficulty: 3,
        knowledge_name: '一元二次函数',
        err_count: 3,
        err_reason: '审题不慎',
        is_star: true,
        status: '待复习',
        last_err_at: '2026-06-23 14:22:00'
      }
    ],
    total: 856,
    page: 1,
...
// Error Codes
// 40001: exam_type 缺失
// 40002: knowledge_id 非法
// 40101: token 无效
// 50001: 服务异常`,
              'http',
            ),
            h3('3.2.3 收藏错题 API 详细规格'),
            code(
`// POST /api/v1/wrong-questions/{id}/star
// Headers
X-Idempotency-Key: <uuid v4>

// Request Body
{
  is_star: true   // true=收藏 false=取消
}
...
// 40901: 幂等键冲突`,
            ...
            h3('3.2.4 学情统计 API 详细规格'),
            code(
`// GET /api/v1/wrong-questions/stats?exam_type=toefl
// Response 200
{
  code: 0,
  data: {
    progress_pct: -18,                  // 本周 vs 上周
    total_done: 856,
    week_done: 30,
    consolidation: {
      wrong_self: 38,                   // 审题不慎
      wrong_reason: 23,                 // 粗心
      right_wrong: 25,                  // 不会做
      wrong_no_idea: 14,                // 时间不够
      main_reason: '审题不慎'
    },
    knowledge_tips: [
      { name: '一元二次函数', mastery_pct: 78, level: '特加强' },
      { name: '函数与图像',   mastery_pct: 72, level: '特加强' },
      { name: '不等式',       mastery_pct: 68, level: '特加强' }
    ]
  }
}`,
              'http',
        ...
        // ========== 状态机设计 ==========
        {
          ...
          blocks: [
            h3('3.3.1 错题模块主流程状态机'),
            code(
`[idle] ──→ [loading_stats] ──→ [ready]
   │            │                  │
   │            ▼                  ▼
   │      [error_stats]      [filtering]
   │            │                  │
   │            ▼                  ▼
   └────→  [retry]           [filter_applied] ──→ [ready]
                                │
                                ▼
                            [batch_selected] ──→ [practicing] ──→ [done]`,
              'text',
            ),
            h3('3.3.2 错题单项生命周期状态机'),
            code(
`States: idle → normal → starred → mastered → normal (循环)
                       ↘ practice_queued → practicing → marked

Transitions:
  normal         + user_star         → starred
  starred        + user_unstar       → normal
  normal/starred + user_mark_master  → mastered
  mastered       + new_err           → normal (err_count += 1)
  any            + system_practice   → practice_queued
  practice_queued+ user_start        → practicing
  practicing     + user_submit       → marked (status='已掌握')
  practicing     + user_skip         → normal (status='待复习')

Side Effects:
  starred    → 写入 is_star=1，记录 starred_at
  mastered   → 写入 status='已掌握'，记录 mastered_at
  new_err    → 写入 err_count += 1，刷新 last_err_at
  marked     → 写入 status='已掌握'，异步更新 student_wrong_stats`,
              'text',
            ),
            h3('3.3.3 收藏请求通用状态机（防抖 + 幂等）'),
            code(
`States: idle → debouncing → requesting → [success | failed | deduped]
                                              ↓
                                          (toast 反馈)
                                              ↓
                                          idle (等待下一次点击)

Debounce: 200ms 合并连续点击
Timeout:  5s 超时视为 failed
Retry:    failed 状态下用户重试 → requesting`,
              'text',
            ),
          ],
        },
...
├── ExamTypeTabs [M001]
│   └── ExamTypeTab × 11
├── LearningStatsPanel [M002]
│   ├── ProgressCard (折线 + 数值)
│   ├── ConsolidationCard (环形 + 归因)
│   └── KnowledgeTipsCard (TOP 3)
├── KnowledgeTreePanel [M003]
│   ├── TreeNode (一级 ×N)
│   │   └── TreeLeaf (二级 ×N)
│   └── TreeCounter
├── WrongQuestionList [M004]
│   ├── FilterBar (级别/题型/难度/筛法/排序/只看收藏)
│   ├── QuestionTable
│   │   └── QuestionRow ×N (含 StarButton, StatusBadge, ActionCell)
│   ├── Pagination
│   └── BatchActionBar (开始练习 / 查看作文)
├── WrongWordsPage (P003)
│   ├── WordStatsPanel [M005]
│   │   ├── WrongBookCard
│   │   └── NewWordBookCard
│   ├── PracticeSettings [M006]
│   │   ├── CountSelector
│   │   └── ReviewModeChips
│   ├── WordFilterBar
│   ├── WordCardGrid [M007]
...
│   └── Pagination
└── GlobalModals
    ├── WordDetailModal (v0.2)
    └── ToastHost`,
              'tsx',
            ),
            h3('3.4.2 状态管理（Pinia Store）'),
            code(
`stores/
├── wrongQuestionsStore
│   ├── state
...
│   │   ├── filters: { level, format, difficulty, sort, is_star }
│   │   ├── list: WrongQuestion[]
│   │   ├── total: number
│   │   ├── page: number
│   │   ├── pageSize: 20 | 50
...
│   │   ├── total: number
│   │   ├── page: number
│   │   ├── sort: 'err_count' | 'last_err_at' | 'word_az'
│   │   ├── isStarOnly: boolean
│   │   ├── keyword: string
│   │   ├── practiceCount: 10 | 20 | 50 | 100
│   │   ├── reviewMode: 'en2cn' | 'listen' | 'spell' | 'cn2en'
│   │   └── loadingState
│   ├── actions
│   │   ├── fetchList()
│   │   ├── fetchStats()
│   │   ├── toggleStar(id, isStar)
│   │   ├── setPracticeConfig(partial)
│   │   └── speak(word, accent)        // 包装 SpeechSynthesis
└── examTypeStore
    ├── list: ExamType[]               // 11 种
    └── default: '新化班2026'`,
              'tsx',
            ...
            h3('3.4.3 关键交互组件约束'),
            ul([
              '<code>ExamTypeTabs</code>：横向滚动容器，sticky top-0，z-index: 10，切换时仅刷新内容区，保留滚动位置。',
              '<code>KnowledgeTreePanel</code>：使用虚拟滚动（react-window），节点数 > 200 时启用，避免长列表卡顿。',
              '<code>QuestionTable</code>：行高固定 56px，使用 <code>content-visibility: auto</code> 优化离屏渲染。',
              '<code>StarButton</code>：乐观更新 + 失败回滚，按钮 loading 状态最长 5s 超时。',
              '<code>Pagination</code>：超过 500 页时显示"1/2/3 ... 10"，含跳转输入框。',
            ]),
          ],
        },
        ...
        {
          id: 'ai-cap',
          title: 'AI 能力工程化设计（v0.3 规划，本期预留）',
          blocks: [
            note(
              '本期范围',
              'v0.1 / v0.2 不启用 AI 能力，本节用于规划 v0.3 上线的 AI 错因分析、错题推荐、错词间隔重复三个 AI 模块。',
              'warn',
            ),
            h3('4.1.1 AI 能力全景'),
            tbl(
              ['能力 ID', '能力名称', '模型', 'Latency SLA', 'Cost Budget', '缓存策略', '上线版本'],
              [
                ['AI-CAP-001', '错因归因', 'GPT-4o-mini', '<2.5s', '≤¥0.03/次', 'Redis 30min', 'v0.3'],
                ['AI-CAP-002', '错题相似推荐', 'GPT-4o-mini', '<3s', '≤¥0.05/次', '题库优先 + LLM 补全', 'v0.3'],
                ['AI-CAP-003', '错词间隔重复', 'GPT-4o-mini', '<2s', '≤¥0.02/次', 'Redis 7天', 'v0.3'],
                ['AI-CAP-004', '薄弱知识点解读', 'GPT-4o-mini', '<4s', '≤¥0.08/次', 'Redis 24h', 'v0.4'],
              ],
            ),
            h3('4.1.2 AI-CAP-001 错因归因（v0.3 详细设计）'),
            h4('Model Role'),
            p('作为"错题诊断专家"，结合错题内容、用户错误历史、归因标签，输出错因分类 + 改进建议。'),
            h4('完整 Prompt（v0.3 上线时启用）'),
            code(
`# Role
你是一位经验丰富的错题诊断专家，擅长分析学生错题背后的根本原因。
...
# Input
{
  ...
  "history_err_reasons": "{{历史归因列表}}",
  "err_count": "{{错课次数}}"
}

# Task
1. 从以下 5 类归因中选出最可能的 1-2 类：
   - 审题不慎（misread）
   - 粗心（careless）
   - 不会做（lack_knowledge）
   - 时间不够（time_pressure）
...
# Output JSON
{
  "primary_reason": "lack_knowledge",
  "secondary_reasons": ["misread"],
  "confidence": 0.86,
  "advice": "建议先复习一元二次函数求根公式，再做 3 道同类题巩固。",
  "knowledge_mastery": 0.42
}`,
              'text',
            ),
            h4('Input / Output Schema'),
            tbl(
              ['字段', '类型', '必填', '说明'],
              [
                ['question_id', 'string', '✓', '错题 ID'],
                ['student_id', 'string', '✓', '学生 ID'],
                ['user_answer', 'string', '✓', '学生作答（截断 500 字）'],
                ['correct_answer', 'string', '✓', '正确答案'],
              ],
            ),
            h4('Latency SLA 与降级策略'),
            ul([
              'P50: <1.5s · P95: <2.5s · P99: <4s · 超时展示"AI 分析中，请稍候"。',
              '主策略：调用 LLM；降级 1：Redis 缓存 <code>ai:attribution:{question_id}:{answer_hash}</code> 30min；降级 2：基于历史 err_reason 取最高频项；降级 3：展示"AI 暂时繁忙"，用户可手动选择归因。',
            ]),
            h3('4.1.3 AI-CAP-002 错题相似推荐（v0.3 详细设计）'),
            p('基于错题 knowledge_id + difficulty，调用题库查询（<code>SELECT * FROM training_questions WHERE knowledge_id=? AND difficulty=? LIMIT 10</code>），不足时调用 LLM 补全。LLM 输出 JSON 题目列表，异步写入题库供下次复用。'),
            h3('4.1.4 AI-CAP-003 错词间隔重复（v0.3 详细设计）'),
            p('基于 SM-2 算法计算每个错词的下次复习时间，调用 LLM 生成"看义选词"题型变体。Redis 存储 <code>word:srs:{student_id}:{word_id}</code>，TTL 30 天。'),
          ],
        },
        ...
        // ========== 验收标准 ==========
        {
          ...
          blocks: [
            ...
            h3('5.3 性能 & SLA 标准'),
            tbl(
              ['模块', 'SLA（P95）', 'Cost Budget', '缓存策略'],
              [
                ['学情统计接口', '<200ms', '—', 'Redis 5min'],
                ['知识点树接口', '<200ms', '—', 'Redis 1h'],
                ['错题列表接口', '<500ms', '—', 'CDN + 游标分页'],
                ['错词列表接口', '<500ms', '—', 'CDN + 游标分页'],
                ['收藏 / 标记掌握', '<300ms', '—', '写穿透 Redis'],
                ['首屏加载（PC）', '<1.5s', '—', 'SSR + 骨架屏'],
                ['列表筛选响应', '<300ms', '—', '前端 debounce 200ms'],
                ['AI 错因归因（v0.3）', '<2.5s', '≤¥0.03/次', 'Redis 30min'],
                ['AI 错题推荐（v0.3）', '<3s', '≤¥0.05/次', '题库优先 + LLM'],
              ],
            ),
            h3('5.4 兼容性'),
            ul([
            ...
            ul([
              ...
              '知识点掌握度建议 = 1 - 知识点错题数 / 知识点总做题数，结果 × 100% 倒序取前 3。',
